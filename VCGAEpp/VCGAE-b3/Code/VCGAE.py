...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.paraA = args.paraA
        self.paraT = args.paraT
        ...
        self.pv_embeddings = self._create_pv_embed()

        if self.node_dropout_flag:
            self.pv_embeddings =  tf.nn.dropout(self.pv_embeddings, 1 - self.mess_dropout[0])
        pv_embeddings = self.pv_embeddings
        self.all_embeddings_pv = pv_embeddings
        self.all_embeddings_pv_all = pv_embeddings
        self.ua_embeddings_pv, self.ia_embeddings_pv = tf.split(self.all_embeddings_pv, [self.n_users, self.n_items], 0)


        self.cart_embeddings = self._create_cart_embed()
        if self.node_dropout_flag:
            self.cart_embeddings =  tf.nn.dropout(self.cart_embeddings, 1 - self.mess_dropout[0])
        cart_embeddings = self.cart_embeddings
        self.all_embeddings_cart = cart_embeddings
        self.pvToCart = cart_embeddings + self.all_embeddings_pv
        self.ua_embeddings_cart, self.ia_embeddings_cart = tf.split(self.all_embeddings_cart, [self.n_users, self.n_items], 0)


        self.buy_embeddings = self._create_lightgcn_embed()
        if self.node_dropout_flag:
            self.buy_embeddings =  tf.nn.dropout(self.buy_embeddings, 1 - self.mess_dropout[0])
        buy_embeddings = self.buy_embeddings
        self.all_embeddings_buy = buy_embeddings + self.pvToCart + self.all_embeddings_pv_all
        ...
        self.loss =  (self.mf_loss) + (self.tradeOff * self.kl_loss)  +  (self.tradeOff_cr *  cr_loss_1) + self.tradeOff_ssl * self.batch_ssl_loss
    ...
    def _create_global_embed(self):
        ...
        self.global_adj = self.norm_adj + self.pv_adj + self.cart_adj
    ...
    def _create_pv_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            for f in range(self.n_fold):
                temp_embed.append(tf.sparse_tensor_dense_matmul(A_fold_hat[f], ego_embeddings))
            #这里temp_emb的形状为[n_fold, （user + item) / n_fold, emb_size]
            #side_emb : [user + item, emb_size]
            side_embeddings = tf.concat(temp_embed, 0)
            #print(k)
            #print(np.array(temp_embed).shape)
            #print(side_embeddings.shape)
            ego_embeddings=side_embeddings
            all_embeddings += [ego_embeddings]
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    f.write(
        'tradeOff_ssl=%.4f, ssl_temp=%.4f, embed_size=%d, lr=%.4f,tradeOff=%.4f,tradeOff_cr=%.4f, layer_size=%s, node_dropout_flag=%s,node_dropout=%s, mess_dropout=%s, regs=%s,tst_file=%s adj_type=%s\n\t%s\n'
        % (args.tradeOff_ssl, args.ssl_temp, args.embed_size, args.lr, args.tradeOff ,args.tradeOff_cr ,args.layer_size, args.node_dropout_flag,args.node_dropout, args.mess_dropout, args.regs,args.tst_file,
